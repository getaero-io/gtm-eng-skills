...
export default definePlay(
  'skill-example-domain-normalizer',
  async (_ctx, input: { domain: string }) => {
    ...
    return {
      domain,
      status: domain ? 'ready' : 'missing_domain',
    };
  },
);
```
...
Company-first contact scratchpads should be written iteratively, but the final file should stay compact: stable source call, cheap normalization/scoring, gated row-level people search, gated email/phone enrichment, then flat scalar export columns with `status`, `source`, and `miss_reason`.

`PlayDataset` is a durable export handle, not an array. Valid bounded inspection methods are `count()`, `peek(limit)`, `materialize(limit)`, lazy `map/filter/slice/take`, async iteration for intentionally small sets, and direct return/export by handle. Do not use `.rows`, `.toArray()`, `.all()`, `.forEach()`, or `.step()` after `.run()`.
