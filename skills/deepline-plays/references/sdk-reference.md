...
| `updateSecretNote` | method | Add, replace, or clear the non-sensitive note attached to one secret. | `secretId: string`<br />`note: string` | `Promise<PlaySecretMetadata>` |
...
| `batch` | `MonitorBatchNamespace` | Yes | Durable bulk audit/deactivation/reactivation/repair with retained progress. |
