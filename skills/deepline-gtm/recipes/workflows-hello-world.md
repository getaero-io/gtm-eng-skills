  ...
  "config": {
    ...
    "commands": [
      ...
      },
      {
        ...
        "tool": "run_javascript",
        "payload": {
          "code": "const payload=(row.build_payload&&row.build_payload.output&&row.build_payload.output.body&&typeof row.build_payload.output.body==='object')?row.build_payload.output.body:{}; return { ok: payload.source === 'cron', preview: payload.message || null };"
        }
      }
    ]
  }
  ...
  "config": {
    ...
    "commands": [
      ...
      },
      {
        ...
        "tool": "run_javascript",
        "payload": {
          "code": "const normalized=(row.normalize_input&&row.normalize_input.output&&row.normalize_input.output.body&&typeof row.normalize_input.output.body==='object')?row.normalize_input.output.body:{}; return { ok: normalized.source === 'webhook', preview: `${normalized.event}:${normalized.company}` };"
        }
      }
    ]
  }
