...
- For SDK plays that process a CSV, the play owns the file contract. Write the play with `ctx.csv(input.file)` (or whatever input field the play declares), then run it with normal JSON input: `deepline plays run enrich.play.ts --input '{"file":"leads.csv"}' --watch`. Do not use a `plays run --csv` shortcut.
