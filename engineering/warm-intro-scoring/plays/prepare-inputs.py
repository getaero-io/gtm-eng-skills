...
fit the Play's 5,000-record bound. Use smaller source cohorts if context alone is
...
def batches(data, targets_per_file=None):
    ...
    require(len(context) + min(size, len(data['targets'])) <= LIMIT,
            f'CSV exceeds {LIMIT} source records; use --targets-per-file or partition source context')
    output = []
