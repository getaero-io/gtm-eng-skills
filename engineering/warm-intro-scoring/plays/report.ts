...
export function renderReview(data:any,config:any=null):string{
 ...
 if(data.coverage){
  ...
  // Optional plain-text notes disclose omitted candidate classes or other coverage limits.
  const notes=Array.isArray(data.coverage.notes)?data.coverage.notes:[];
  const note=`<section aria-label="Source coverage"><h2>Source coverage</h2><p>${missing.length} unresolved targets. ${orphans.length} prior review states retained without a current path. Export completeness: ${esc(data.coverage.export_completeness||'not verified')}.</p><ul>${missing.map((r:any)=>{const t=r.target||{},name=esc(t.name||t.id||'Unknown target'),url=t.linkedin_url;const identity=typeof url==='string'&&/^https:\/\/(?:www\.)?linkedin\.com\/in\/[^/?#\s]+\/?$/.test(url)?`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">${name}</a>`:name;return `<li>${identity} — ${esc([t.title||'Job title unknown',t.company].filter(Boolean).join(' · '))}: ${esc(r.reason)}${r.domain_check?`<div>Expected account: ${esc(r.domain_check.expected_domain||'Missing')} · Profile account: ${esc(r.domain_check.profile_domain||'Missing')} · ${esc(r.domain_check.status)}</div>`:''}</li>`;}).join('')}</ul>${notes.length?`<ul>${notes.map((n:any)=>`<li>${esc(n)}</li>`).join('')}</ul>`:''}</section>`;
