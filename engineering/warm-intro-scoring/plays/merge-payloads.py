...
def merge(exports, previous=None, expected_keys=None):
        ...
        with Path(source).open(newline='') as handle:
            for row in csv.DictReader(handle):
                ...
                for key, value in payload.get('target_research', {}).items():
                    ...
                    research[key] = value
                coverage = result.get('coverage', payload.get('coverage', {}))
                fields = ('connectors', 'matched_profiles', 'self_pairs_excluded', 'paths')
                if any(k in coverage for k in fields):
                    if not all(type(coverage.get(k)) is int and coverage[k] >= 0 for k in fields):
                        raise ValueError('Incomplete or invalid source coverage counts')
                    expected_paths = coverage['connectors'] * coverage['matched_profiles'] - coverage['self_pairs_excluded']
                    if expected_paths != len(payload['paths']) or coverage['paths'] != expected_paths:
                        raise ValueError('Source coverage does not reconcile with exported paths')
                source_coverage.append({'key': row.get('key'), 'coverage': coverage})
                notes.extend(n for n in coverage.get('notes', []) if isinstance(n, str))
                unresolved.extend(coverage.get('unresolved', []))
    ...
    def fingerprint(path, registry):
        features = path.get('features', {})
        ids = sorted({eid for feature in features.values() for eid in feature.get('evidence_ids', [])})
        value = {'features': features, 'evidence': [registry.get(eid) for eid in ids]}
        return 'evidence-v2:' + hashlib.sha256(json.dumps(value, sort_keys=True,
                              separators=(',', ':'), allow_nan=False).encode()).hexdigest()
    previous_evidence = {e['id']: e for e in (previous or {}).get('evidence', [])}
    ledger = {}
    if previous is not None:
        ...
        for old in previous.get('review_state', []) + previous.get('paths', []):
            ...
            state = dict(id=old['id'], connector_id=key[0], target_id=key[1],
                         ...
                         feature_hash=old.get('feature_hash', fingerprint(old, previous_evidence)))
    ...
    for path in paths.values():
        ...
        old = ledger.get(key)
        if old:
            state = 'blocked_declined' if path['review_status'] == 'blocked_declined' else old['review_status']
            if state == 'ready_for_human_review' and old['feature_hash'] != fingerprint(path, evidence):
                state = 'needs_confirmation'
            path['review_status'] = state
        ledger[key] = dict(id=path['id'], connector_id=key[0], target_id=key[1],
                           review_status=path['review_status'], feature_hash=fingerprint(path, evidence))
    ...
    combined.update(paths=list(paths.values()), evidence=list(evidence.values()),
                    target_research=research, review_state=list(ledger.values()), coverage={'unresolved': unresolved, 'review_states_without_paths': orphaned, 'notes': list(dict.fromkeys(notes)), 'source_coverage': source_coverage, 'export_completeness': 'target_exports_verified' if expected_keys is not None else 'not_verified_without_source_manifest', 'exported_targets': len(export_keys)})
