...
## PR refresh review — 2026-09-24

Two independent read-only reviews covered linked profiles, report rendering, employee coverage and refresh holds. Reproduced findings and changes:

| Case | Result |
|---|---|
| Old ready state plus new decline | Fixed: the decline wins. |
| Same features and IDs, changed source detail/date | Fixed: readiness hashes include referenced source records. Old hash versions reset readiness. |
| Source path count differs from exported paths | Fixed: merge rejects inconsistent counts. Coverage notes and per-export counts survive. |
| All target keys exported, partial employee roster | The status now says `target_exports_verified`; it makes no employee completeness claim. Roster completeness still requires provider receipts and reconciliation. |
| Connector shards repeat the same target key | Unsupported and rejected. Documentation no longer recommends this merge path. Use local TypeScript extraction when shared source context exceeds 5,000 records. |
| Unsafe profile URL or HTML in a name | URL validation rejects unsafe destinations; names render as text. Linked profiles use a new tab and `noopener noreferrer`. |

Regression cases are in `tests/test_prepare_inputs.py`, `tests/features.test.ts`, `scripts/test_tuning.py` and `scripts/test_tuning_browser.cjs`. No live employee pull or introduction send was performed. No independent source-truth or conversion claim is made.

The reviewed PR head also had a failing repository CI test: `tests/lib/integrations/fullenrich-v2.test.ts`, “keeps UNKNOWN results pending in the synchronous async-job handler” (`initialUsageUsd` was undefined). That file is outside this recipe's diff. Do not describe the entire PR as green on the strength of offline recipe checks.
