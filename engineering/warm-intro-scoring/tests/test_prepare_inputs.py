...
class InputPackagingTests(unittest.TestCase):
    ...
    def test_refresh_preserves_new_decline_and_invalidates_changed_evidence(self):
        old = dict(id='p', connector_id='c', target_id='t', review_status='ready_for_human_review', features={'work':{'value':1,'evidence_ids':['e']}})
        original = dict(id='e', source='fixture', observed_at='2026-01-01', detail='Original fact')
        prior = {'paths':[old], 'evidence':[original]}
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp)/'export.csv'
            def write(status, record):
                with path.open('w',newline='') as f:
                    w=csv.DictWriter(f,fieldnames=['key','result']);w.writeheader();w.writerow({'key':'target:0','result':json.dumps({'payload':{'paths':[dict(old,review_status=status)],'evidence':[record]}})})
            write('blocked_declined', original)
            self.assertEqual(merger.merge([path],prior)['paths'][0]['review_status'],'blocked_declined')
            write('needs_confirmation', dict(original,detail='Changed fact'))
            self.assertEqual(merger.merge([path],prior)['paths'][0]['review_status'],'needs_confirmation')
            write('needs_confirmation', original)
            self.assertEqual(merger.merge([path],prior)['paths'][0]['review_status'],'ready_for_human_review')

    def test_source_coverage_survives_and_truncated_paths_fail(self):
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/'export.csv'
            coverage={'connectors':5,'matched_profiles':1,'self_pairs_excluded':0,'paths':5,'unresolved':[],'notes':['Partial employee roster']}
            payload={'paths':[],'evidence':[]}
            def write():
                with path.open('w',newline='') as f:
                    w=csv.DictWriter(f,fieldnames=['key','result']);w.writeheader();w.writerow({'key':'target:0','result':json.dumps({'payload':payload,'coverage':coverage})})
            write()
            with self.assertRaisesRegex(ValueError,'coverage'):merger.merge([path],expected_keys=['target:0'])
            coverage.update(connectors=0,paths=0);write()
            result=merger.merge([path],expected_keys=['target:0'])
            self.assertEqual(result['coverage']['notes'],coverage['notes'])
            self.assertEqual(result['coverage']['source_coverage'][0]['coverage'],coverage)
            self.assertEqual(result['coverage']['export_completeness'],'target_exports_verified')
