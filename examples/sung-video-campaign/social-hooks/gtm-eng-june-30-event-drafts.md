# GTM Engineering June 30 Event Drafts

Event: `https://luma.com/fx5v8axr`
...
The interesting GTM + AI work is moving from "write me an email" to internal workflows:

```text
signup -> CRM state -> suppression -> sequence
warehouse signal -> owner context -> task
event check-in -> enrichment -> follow-up
usage spike -> expansion assist
```

That is the session frame.
...
```text
Most GTM + AI demos stop at:

"look, the agent wrote an email."

That is not where the real work is.

The real work is:

- Should this person be sequenced?
- Is there an open opportunity?
- Who owns the account?
- Did the CRM already know this?
- What product usage made this worth a human touch?
...
```text
The best GTM engineering workflows are usually a little boring.

New signup?
Check HubSpot.
Suppress customers and open opps.
Add the right lifecycle context.
Enroll in the right sequence.
...
```text
The useful GTM + AI workflows are usually not "agent wrote cold email."

They are:

signup -> suppressions -> HubSpot sequence
product usage -> CRM context -> rep task
event check-in -> enrichment -> follow-up
...
- is there an open opportunity?
- who owns the account?
- what product usage made this worth action?
- what gets written back?
```
...
new signup -> HubSpot context -> suppressions -> right sequence

That one workflow is more useful than a lot of AI SDR demos.
```

```text
We will talk through workflows like:

product usage -> CRM task
event check-in -> follow-up
warehouse signal -> campaign draft
usage drop -> CS alert
personal email signup -> identity review
```
...
```text
Teams like Ramp, OpenAI, and Notion are not just asking AI to write copy.

The interesting work is internal GTM flows:
...
```text
The best GTM + AI workflows are usually boring.
...
```text
GTM engineering is the work between:
...
1. LinkedIn 2 as the main announcement.
2. X 2 the same morning.
3. Threads 2 later that day.
4. X thread 48-72 hours later.
5. LinkedIn 4 one week before the event.
